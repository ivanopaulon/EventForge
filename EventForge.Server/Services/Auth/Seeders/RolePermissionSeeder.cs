...
/// Kept as a separate unit to allow easier reuse.
/// </summary>
public static class RolePermissionSeeder
{
    public static async Task<bool> SeedAsync(EventForgeDbContext dbContext, ILogger logger, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var toAddPermissions = defaultPermissions
                .Where(p => !existingNamesSet.Contains(p.Name))
                .Select(p => { p.CreatedBy = "system"; p.CreatedAt = now; return p; })
                .ToList();

            if (toAddPermissions.Count > 0)
            {
                dbContext.Permissions.AddRange(toAddPermissions);
                _ = await dbContext.SaveChangesAsync(cancellationToken);
                logger.LogWarning("Added {Count} missing system permissions", toAddPermissions.Count);
            }
            ...
            var existingRoleNames = await dbContext.Roles.Select(r => r.Name).ToListAsync(cancellationToken);
            var existingRolesSet = existingRoleNames.ToHashSet(StringComparer.OrdinalIgnoreCase);

            var rolesToAdd = defaultRoles
                .Where(r => !existingRolesSet.Contains(r.Name))
                .Select(r => { r.CreatedBy = "system"; r.CreatedAt = now; return r; })
                .ToList();
            ...
            if (rolesToAdd.Count > 0)
            {
                ...
                _ = await dbContext.SaveChangesAsync(cancellationToken);
                logger.LogWarning("Added {Count} missing system roles", rolesToAdd.Count);
            }

            // Assign all permissions to Admin and SuperAdmin (idempotent)
            ...
            async Task EnsureRoleHasAllPermissions(string roleName)
            {
                ...
                if (toAdd.Count > 0)
                {
                    ...
                    _ = await dbContext.SaveChangesAsync(cancellationToken);
                    logger.LogWarning("Assigned {Count} missing permissions to role '{Role}'", toAdd.Count, roleName);
                }
            }
