...
/// </summary>
public class FiscalMappingService(EventForgeDbContext context, ILogger<FiscalMappingService> logger) : IFiscalMappingService
{
    ...
    public int GetVatFiscalCode(Guid vatRateId)
    {
        try
        {
            ...
            if (vatRate is null)
            {
                logger.LogWarning("VAT rate {VatRateId} not found. Using default fiscal code 1.", vatRateId);
                return 1;
            }
        ...
        {
            logger.LogError(ex, "Error retrieving fiscal code for VAT rate {VatRateId}. Using default code 1.", vatRateId);
            return 1;
        }
    }
    ...
    public int GetVatFiscalCode(decimal vatPercentage, int? configuredCode = null)
    {
        ...
        {
            logger.LogDebug("Using configured fiscal code {FiscalCode} for VAT {Percentage}%", configuredCode.Value, vatPercentage);
        ...
        if (configuredCode is null)
        {
            logger.LogDebug("Using automatic fiscal code {FiscalCode} for VAT {Percentage}% (not configured)", fiscalCode, vatPercentage);
        }
        else
        {
            logger.LogWarning("Configured fiscal code {ConfiguredCode} for VAT {Percentage}% is invalid. Using fallback code {FiscalCode}.", configuredCode, vatPercentage, fiscalCode);
        }
    ...
    public int GetPaymentFiscalCode(Guid paymentMethodId)
    {
        try
        {
            ...
            {
                logger.LogWarning("Payment method {PaymentMethodId} not found. Using default fiscal code 1.", paymentMethodId);
                return 1;
            }
        ...
        {
            logger.LogError(ex, "Error retrieving fiscal code for payment method {PaymentMethodId}. Using default code 1.", paymentMethodId);
            return 1;
        }
    }
    ...
    public int GetPaymentFiscalCode(string paymentCode, int? configuredCode = null)
    {
        ...
        if (configuredCode.HasValue && configuredCode.Value >= 1 && configuredCode.Value <= 10)
        {
            logger.LogDebug("Using configured fiscal code {FiscalCode} for payment method {PaymentCode}", configuredCode.Value, paymentCode);
            return configuredCode.Value;
        }
        ...
        if (configuredCode is null)
        {
            logger.LogDebug("Using automatic fiscal code {FiscalCode} for payment method {PaymentCode} (not configured)", fiscalCode, paymentCode);
        }
        else
        {
            logger.LogWarning("Configured fiscal code {ConfiguredCode} for payment method {PaymentCode} is invalid. Using fallback code {FiscalCode}.", configuredCode, paymentCode, fiscalCode);
        }
