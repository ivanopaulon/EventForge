...
public class DocumentType : AuditableEntity
{
    ...
    /// <summary>
    /// When <c>true</c>, this document type represents a stock transfer between two warehouses.
    /// The rebuild procedure will generate both an <c>Outbound</c> movement from the source
    /// warehouse/location and an <c>Inbound</c> movement to the destination warehouse/location
    /// for each document row, instead of a single movement.
    /// </summary>
    [Display(Name = "Is Transfer Document", Description = "Whether this document represents a transfer between warehouses (generates both Outbound and Inbound movements).")]
    public bool IsTransferDocument { get; set; } = false;

    /// <summary>
    /// Default <see cref="Warehouse.StockMovementReason"/> assigned to movements generated by this
    /// document type during the rebuild procedure.  When <c>null</c> the procedure falls back to
    /// the legacy heuristic (Purchase for inbound documents, Sale for outbound).
    /// </summary>
    [Display(Name = "Default Movement Reason", Description = "Default reason assigned to stock movements generated by the rebuild procedure for this document type.")]
    public Warehouse.StockMovementReason? DefaultMovementReason { get; set; }
}