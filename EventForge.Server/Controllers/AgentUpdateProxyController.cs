    ...
    ILogger<AgentUpdateProxyController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> GetPendingInstalls(CancellationToken ct)
    {
        ...
        catch (AgentNotConfiguredException ex) { return StatusCode(503, new { Detail = ex.Message }); }
        catch (Exception ex) { logger.LogError(ex, "Error fetching pending installs from Agent"); return StatusCode(502, new { Detail = ex.Message }); }
    }
    ...
    public async Task<IActionResult> TriggerInstallNow([FromBody] AgentInstallNowProxyRequest request, CancellationToken ct)
    {
        ...
        catch (Exception ex) { logger.LogError(ex, "Error sending InstallNow to Agent"); return StatusCode(502, new { Detail = ex.Message }); }
    }
    ...
    public async Task<IActionResult> TriggerUnblockQueue([FromBody] AgentUnblockQueueProxyRequest request, CancellationToken ct)
    {
        ...
        catch (Exception ex) { logger.LogError(ex, "Error sending UnblockQueue to Agent"); return StatusCode(502, new { Detail = ex.Message }); }
    }
}
