...
namespace Prym.Tests.RouteAnalysis;
...
public class RouteAnalysisTests
{
    ...
    {
        // Arrange
        var controllersPath = Environment.GetEnvironmentVariable("CONTROLLERS_PATH") ?? "Prym.Server/Controllers";
        ...
        // Walk up until we find Prym.sln or reach a reasonable limit
        for (int i = 0; i < 10; i++)
        {
            if (File.Exists(Path.Combine(solutionRoot, "Prym.sln")))
                break;
        ...
        {
            Console.WriteLine("Prym Route Conflict Analyzer");
            Console.WriteLine("===================================");
            Console.WriteLine();
