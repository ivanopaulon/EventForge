...
using Prym.Server.Data.Entities.Products;
using Prym.Server.Services.Products;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Logging;
using Moq;

namespace Prym.Tests.Services;
...
{
    private readonly DbContextOptions<PrymDbContext> _dbContextOptions;

    public SupplierProductAssociationTests()
    {
        _dbContextOptions = new DbContextOptionsBuilder<PrymDbContext>()
    ...
    {
        // Arrange
        using var context = new PrymDbContext(_dbContextOptions);
    ...
    private ProductService CreateProductService(PrymDbContext context)
    {
        var loggerMock = new Mock<ILogger<ProductService>>();
        var auditLogServiceMock = new Mock<Prym.Server.Services.Audit.IAuditLogService>();
        var tenantContextMock = new Mock<Prym.Server.Services.Tenants.ITenantContext>();
        var codeGeneratorMock = new Mock<Prym.Server.Services.CodeGeneration.IDailyCodeGenerator>();

        // Setup tenant context to return a valid tenant ID
        tenantContextMock.Setup(x => x.CurrentTenantId).Returns((Guid?)Guid.NewGuid());

        var priceHistoryServiceMock = new Mock<Prym.Server.Services.PriceHistory.ISupplierProductPriceHistoryService>();
