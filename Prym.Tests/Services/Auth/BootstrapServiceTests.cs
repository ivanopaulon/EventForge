...
using Prym.Server.Data.Entities.Store;
using Prym.Server.Services.Auth;
using Prym.Server.Services.Auth.Seeders;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Configuration;
using Microsoft.Extensions.Logging;

namespace Prym.Tests.Services.Auth;

[Trait("Category", "Unit")]
public class BootstrapServiceTests
{
    private static BootstrapService CreateBootstrapService(PrymDbContext context, IConfiguration config)
    {
    ...
    public async Task EnsureAdminBootstrappedAsync_WithEmptyDatabase_ShouldCreateInitialData()
    {
        ...
        await using var context = new PrymDbContext(options);
    ...
    public async Task EnsureAdminBootstrappedAsync_WithExistingTenants_ShouldCreateUsersAndSeedEntities()
    {
        ...
        var existingTenant = new Prym.Server.Data.Entities.Auth.Tenant
        {
            Name = "ExistingTenant",
            Code = "existing",
    ...
    public async Task EnsureAdminBootstrappedAsync_RunTwice_ShouldNotDuplicateBaseEntities()
    {
        ...
        await using var context2 = new PrymDbContext(options);
        ...
        await using var verifyContext = new PrymDbContext(options);
