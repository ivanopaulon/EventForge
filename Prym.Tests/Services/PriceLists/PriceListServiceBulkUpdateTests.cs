...
public class PriceListServiceBulkUpdateTests
{
    ...
    private PrymDbContext CreateDbContext()
    {
        var options = new DbContextOptionsBuilder<PrymDbContext>()
            .UseInMemoryDatabase(databaseName: Guid.NewGuid().ToString())
            .Options;

        return new PrymDbContext(options);
    }

    private async Task<(PrymDbContext context, PriceList priceList, List<PriceListEntry> entries)> SeedTestDataAsync()
    {
