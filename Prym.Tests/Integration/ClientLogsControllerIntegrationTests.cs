...
namespace Prym.Tests.Integration;
        ...
        var client = _factory.WithWebHostBuilder(builder =>
        {
            _ = builder.ConfigureServices(services =>
            {
                ...
                var descriptor = services.SingleOrDefault(
                    d => d.ServiceType == typeof(DbContextOptions<PrymDbContext>));
                if (descriptor != null)
                    _ = services.Remove(descriptor);

                _ = services.AddDbContext<PrymDbContext>(options =>
                {
                    _ = options.UseInMemoryDatabase("TestDb_ClientLogs");
                });
    ...
    public async Task LogClientBatch_WithoutAuthentication_ShouldSucceed()
    {
        ...
        var client = _factory.WithWebHostBuilder(builder =>
        {
            _ = builder.ConfigureServices(services =>
            {
                ...
                _ = services.AddDbContext<PrymDbContext>(options =>
                {
                    _ = options.UseInMemoryDatabase("TestDb_ClientLogsBatch");
                });
    ...
    public async Task LogClientEntry_WithInvalidData_ShouldReturnBadRequest()
    {
        ...
        var client = _factory.WithWebHostBuilder(builder =>
        {
            _ = builder.ConfigureServices(services =>
            {
                ...
                _ = services.AddDbContext<PrymDbContext>(options =>
                {
                    _ = options.UseInMemoryDatabase("TestDb_ClientLogsInvalid");
                });
