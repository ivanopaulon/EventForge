...
builder.Services.Configure<Prym.Server.Configuration.PaginationSettings>(
    builder.Configuration.GetSection(Prym.Server.Configuration.PaginationSettings.SectionName));

// Add API Controllers support
builder.Services.AddControllers(options =>
{
    options.ModelBinderProviders.Insert(0, new Prym.Server.ModelBinders.PaginationModelBinderProvider());
    // Add FluentValidation filter
    options.Filters.Add<Prym.Server.Filters.FluentValidationFilter>();
})
...
builder.Services.AddSingleton<Prym.Server.Services.Caching.ICacheService, Prym.Server.Services.Caching.CacheService>();

// Register cache invalidation service for Output Cache
builder.Services.AddScoped<Prym.Server.Services.Caching.ICacheInvalidationService, Prym.Server.Services.Caching.CacheInvalidationService>();

// Register Setup Wizard services
builder.Services.AddScoped<Prym.Server.Services.Setup.IFirstRunDetectionService, Prym.Server.Services.Setup.FirstRunDetectionService>();
builder.Services.AddScoped<Prym.Server.Services.Setup.ISqlServerDiscoveryService, Prym.Server.Services.Setup.SqlServerDiscoveryService>();
builder.Services.AddScoped<Prym.Server.Services.Setup.ISetupWizardService, Prym.Server.Services.Setup.SetupWizardService>();

// Register Configuration services
builder.Services.AddScoped<Prym.Server.Services.Configuration.IPortConfigurationService, Prym.Server.Services.Configuration.PortConfigurationService>();
builder.Services.AddScoped<Prym.Server.Services.Configuration.IBrandingService, Prym.Server.Services.Configuration.BrandingService>();

// Register Update services
builder.Services.AddSingleton<Prym.Server.Services.Updates.IUpdateHubProxyService, Prym.Server.Services.Updates.UpdateHubProxyService>();
builder.Services.AddSingleton<Prym.Server.Services.Updates.IAgentUpdateProxyService, Prym.Server.Services.Updates.AgentUpdateProxyService>();

// UpdatesAvailableRefreshService — broadcasts ReadyToDeploy package count to SuperAdmin clients
builder.Services.AddSingleton<Prym.Server.Services.Updates.UpdatesAvailableRefreshService>();
builder.Services.AddHostedService(sp => sp.GetRequiredService<Prym.Server.Services.Updates.UpdatesAvailableRefreshService>());

// Agent monitor — singleton background service (probes Agent, auto-restarts if unreachable > threshold)
builder.Services.AddSingleton<Prym.Server.Services.AgentMonitorService>();
builder.Services.AddHostedService(sp => sp.GetRequiredService<Prym.Server.Services.AgentMonitorService>());

// Register Dashboard services
builder.Services.AddScoped<Prym.Server.Services.Dashboard.IServerStatusService, Prym.Server.Services.Dashboard.ServerStatusService>();
builder.Services.AddScoped<Prym.Server.Services.Dashboard.IPerformanceMetricsService, Prym.Server.Services.Dashboard.PerformanceMetricsService>();

// Register Monitoring services (Sprint 4 — Fase 6 Optimization)
builder.Services.AddSingleton<Prym.Server.Services.Monitoring.IMonitoringMetricsService, Prym.Server.Services.Monitoring.MonitoringMetricsService>();
builder.Services.AddScoped<Prym.Server.Services.Monitoring.IMonitoringService, Prym.Server.Services.Monitoring.MonitoringService>();

// Register Hosted Services
builder.Services.AddHostedService<Prym.Server.HostedServices.LogCleanupService>();
builder.Services.AddHostedService<Prym.Server.HostedServices.PerformanceCollectorService>();
...
builder.Services.AddSwaggerGen(c =>
{
    ...
    c.OperationFilter<Prym.Server.Swagger.SwaggerAuthorizeCheckOperationFilter>();
...
using (var scope = app.Services.CreateScope())
{
    ...
    Prym.Server.Startup.DependencyValidationService.ValidateDependencies(
...
app.UseMiddleware<Prym.Server.Middleware.SetupWizardMiddleware>();
...
app.UseMiddleware<Prym.Server.Middleware.MaintenanceMiddleware>();
...
app.MapHub<Prym.Server.Hubs.ConfigurationHub>("/hubs/configuration");
app.MapHub<Prym.Server.Hubs.FiscalPrinterHub>("/hubs/fiscal-printer");
app.MapHub<Prym.Server.Hubs.UpdateNotificationHub>("/hubs/update-notifications");
