...
/// </summary>
public class PrymDbContextFactory : IDesignTimeDbContextFactory<PrymDbContext>
{
    public PrymDbContext CreateDbContext(string[] args)
    {
        // Base path = cartella del progetto Prym.Server
        ...
        if (!string.IsNullOrEmpty(connectionString))
        {
            ...
            Console.WriteLine($"[PrymDbContextFactory] USING ConnectionString name='{usedName}' -> {masked}");
        }
        else
        {
            Console.WriteLine("[PrymDbContextFactory] No connection string found in appsettings.* or environment variables.");
        }

        var optionsBuilder = new DbContextOptionsBuilder<PrymDbContext>();
        ...
        else
        {
            ...
            optionsBuilder.UseSqlServer("Server=localhost;Database=PrymDummy;Trusted_Connection=True;TrustServerCertificate=True;");
        }

        return new PrymDbContext(optionsBuilder.Options);
    }
