    ...
    ILogger<WarehouseFacade> logger) : IWarehouseFacade
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var allRows = documents.SelectMany(d => d.Rows?.Where(r => !r.IsDeleted) ?? Enumerable.Empty<Prym.Server.Data.Entities.Documents.DocumentRow>()).ToList();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var targetRowLookup = new Dictionary<(Guid?, Guid?), Prym.Server.Data.Entities.Documents.DocumentRow>(targetRows.Count);
            foreach (var row in targetRows)
                targetRowLookup.TryAdd((row.ProductId, row.LocationId), row);

            var newRows = new List<Prym.Server.Data.Entities.Documents.DocumentRow>();
            ...
            if (sourceDocIds.Count > 0)
            {
                ...
                foreach (var sourceRow in sourceRows)
                {
                    ...
                    else
                    {
                        ...
                        var newRow = new Prym.Server.Data.Entities.Documents.DocumentRow
                        {
                            Id = Guid.NewGuid(),
