    ...
    IWebHostEnvironment environment) : IBackupService
{
    ...
    {
        var backup = await context.BackupOperations.FindAsync(new object[] { backupId }, ct);
        ...
        try
        {
            ...
            var backupDir = Path.Combine(environment.ContentRootPath, "Backups");
            ...
            logger.LogInformation("Backup completed successfully: {BackupId}, File: {FilePath}, Size: {Size} bytes",
                backupId, filePath, fileInfo.Length);
        }
        catch (OperationCanceledException)
        {
            logger.LogInformation("Backup operation {BackupId} was cancelled", backupId);
            backup.Status = "Cancelled";
            backup.CompletedAt = DateTime.UtcNow;
            backup.ModifiedAt = DateTime.UtcNow;
            backup.ModifiedBy = "System";
        ...
        catch (Exception ex)
        {
            ...
            logger.LogError(ex, "Backup failed: {BackupId}", backupId);
        }
    }
    ...
    {
        var auditLogs = await context.EntityChangeLogs
            .AsNoTracking()
            .ToListAsync(ct);
    ...
    private async Task<string> GetUserDisplayNameAsync(Guid userId, CancellationToken ct = default)
    {
        ...
        return user is not null ? $"{user.FirstName} {user.LastName}".Trim() : "Unknown User";
    }
