...
namespace Prym.Server.Services.Station;
    ...
    ILogger<StationService> logger) : IStationService
{
    ...
    public async Task<StationDto> CreateStationAsync(CreateStationDto createStationDto, string currentUser, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var station = new Prym.Server.Data.Entities.StationMonitor.Station
            {
    ...
    private static StationDto MapToStationDto(Prym.Server.Data.Entities.StationMonitor.Station station, int printerCount)
    {
