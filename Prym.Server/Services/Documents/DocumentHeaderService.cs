...
namespace Prym.Server.Services.Documents;
    ...
    ILogger<DocumentHeaderService> logger) : IDocumentHeaderService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            documentHeader.ApprovalStatus = Prym.Server.Data.Entities.Documents.ApprovalStatus.Approved;
    ...
    private IQueryable<DocumentHeader> BuildDocumentHeaderQuery(DocumentHeaderQueryParameters parameters)
    {
        ...
        if (parameters.PaymentStatus.HasValue)
            query = query.Where(dh => dh.PaymentStatus == (Prym.Server.Data.Entities.Documents.PaymentStatus)parameters.PaymentStatus.Value);

        if (parameters.ApprovalStatus.HasValue)
            query = query.Where(dh => dh.ApprovalStatus == (Prym.Server.Data.Entities.Documents.ApprovalStatus)parameters.ApprovalStatus.Value);
