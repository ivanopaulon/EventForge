# Prym Deployment Guide - IIS Configuration

## Overview

This guide provides comprehensive instructions for deploying Prym to Internet Information Services (IIS) with automatic startup and proper configuration. When publishing `Prym.Server`, the Blazor WebAssembly client (`Prym.Client`) is automatically published and its output is copied into the server's `wwwroot` directory. This allows the server to host the client as static files, enabling a unified deployment to IIS or other web servers.

## How It Works

The `Prym.Server.csproj` file contains an MSBuild target (`PublishBlazorClient`) that runs before the server publish process. This target:

1. Publishes the `Prym.Client` project using the same configuration (Debug/Release) as the server
...
```bash
dotnet publish Prym.Server -c Release -o ./publish-output
```
...
Publish the Prym.Server project in Release configuration:
...
1. Create a directory on the IIS server (e.g., `C:\inetpub\Prym`)
...
3. Ensure the IIS Application Pool identity has read/write permissions:
   ...
   icacls "C:\inetpub\Prym" /grant "IIS_IUSRS:(OI)(CI)M" /T
   ```
4. Create a `logs` subdirectory for stdout logging:
   ```powershell
   mkdir "C:\inetpub\Prym\logs"
   icacls "C:\inetpub\Prym\logs" /grant "IIS_IUSRS:(OI)(CI)M" /T
   ```
...
3. **Configure Application Pool Advanced Settings:**
   ...
   Right-click on `PrymAppPool` → "Advanced Settings" and configure:
...
2. Configure:
   - **Site name**: Prym
   - **Application pool**: PrymAppPool (select the one created above)
   - **Physical path**: `C:\inetpub\Prym`
   - **Binding**:
     - Type: http or https
...
2. Configure:
   - **Alias**: Prym
   - **Application pool**: PrymAppPool
   - **Physical path**: `C:\inetpub\Prym`
3. Click OK
4. **Important**: If deploying as a virtual directory, update `<base href="/Prym/" />` in the published `wwwroot/index.html`
...
1. In IIS Manager, select your site (e.g., Prym)
...
Set-ItemProperty "IIS:\Sites\Prym" -Name applicationDefaults.preloadEnabled -Value $true

# Verify
Get-ItemProperty "IIS:\Sites\Prym" -Name applicationDefaults.preloadEnabled
```

**Method 3: Using appcmd.exe**

```cmd
%windir%\system32\inetsrv\appcmd.exe set app "Prym/" /preloadEnabled:true
```
...
2. Check Windows Task Manager - w3wp.exe process should be running for PrymAppPool
3. Check Application logs at `C:\inetpub\Prym\logs\stdout_*.log`
...
Edit `C:\inetpub\Prym\web.config`:

```xml
<aspNetCore processPath="dotnet" 
            arguments=".\Prym.Server.dll" 
...
- **`arguments=".\Prym.Server.dll"`**: The application DLL to execute
...
1. **Check Application Pool Status:**
   ```powershell
   Get-IISAppPool -Name "PrymAppPool"
...
3. **Check stdout Logs:**
   ...
   Get-Content "C:\inetpub\Prym\logs\stdout_*.log" -Tail 50
   ```
...
| **Configuration Location** | `Prym.Server/Properties/launchSettings.json` | IIS Manager → Site → Bindings |
...
| Aspect | Development | Production |
|--------|-------------|------------|
| **Base Path** | Project directory | IIS site physical path (e.g., `C:\inetpub\Prym`) |
...
2. Application Pool `PrymAppPool` starts (Start Mode: AlwaysRunning)
...
3. **Check stdout Logs:**
   ```powershell
   Get-Content "C:\inetpub\Prym\logs\stdout_*.log" -Tail 100
   ```
...
1. **Verify index.html exists:**
   ```powershell
   Test-Path "C:\inetpub\Prym\wwwroot\index.html"
   ```
...
4. **Check MIME types:**
   ...
   Get-WebConfigurationProperty -PSPath "IIS:\Sites\Prym" -Filter "//staticContent/mimeMap[@fileExtension='.wasm']" -Name "mimeType"
   ```
...
1. **Verify application is running:**
   ```powershell
   Get-Process w3wp | Where-Object {$_.Modules.ModuleName -like "*Prym*"}
   ```
...
1. **Check Idle Timeout:**
   ...
   ```powershell
   Set-ItemProperty "IIS:\AppPools\PrymAppPool" -Name processModel.idleTimeout -Value "00:00:00"
   ```
...
2. **Check QZ Tray library version in the client:**
   ...
   Get-ChildItem "C:\inetpub\Prym\wwwroot" -Recurse -Filter "*qz-tray*"
   ```

3. **Update QZ Tray JavaScript library:**
   - The application should use the npm package `qz-tray` version 2.2.x or later
   - Check `Prym.Client/package.json` for qz-tray version
...
1. **Grant IIS_IUSRS permissions:**
   ```powershell
   icacls "C:\inetpub\Prym" /grant "IIS_IUSRS:(OI)(CI)M" /T
   icacls "C:\inetpub\Prym\logs" /grant "IIS_IUSRS:(OI)(CI)F" /T
   ```
...
2. **Grant database permissions:**
   ...
   - Grant permissions to `IIS APPPOOL\PrymAppPool`

3. **Check file system permissions:**
   ```powershell
   Get-Acl "C:\inetpub\Prym" | Format-List
   ```
...
1. **Backup current deployment:**
   ```powershell
   # Stop the site
   Stop-WebSite -Name "Prym"
   
   # Backup files
   Copy-Item "C:\inetpub\Prym" -Destination "C:\inetpub\Backups\Prym_$(Get-Date -Format 'yyyyMMdd_HHmmss')" -Recurse
   ```
...
3. **Deploy update:**
   ...
   Copy-Item ".\publish-output\*" -Destination "C:\inetpub\Prym" -Recurse -Force -Exclude web.config
   ```

4. **Restart site:**
   ```powershell
   Start-WebSite -Name "Prym"
   ```
...
jobs:
  deploy:
    ...
    - name: Publish
      run: dotnet publish Prym.Server -c Release -o ./publish
    - name: Deploy to IIS
      run: |
        Stop-WebSite -Name "Prym"
        Copy-Item .\publish\* C:\inetpub\Prym -Recurse -Force
        Start-WebSite -Name "Prym"
```
...
While this guide focuses on IIS, Prym can also be deployed in Docker:
...
This guide covers the complete process of deploying Prym to IIS with automatic startup:
