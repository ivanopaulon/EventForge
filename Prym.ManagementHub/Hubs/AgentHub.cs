    ...
    ManagementHubOptions hubOptions) : Hub
{
    public override async Task OnConnectedAsync()
    {
        ...
        {
            logger.LogError(ex, "Error in OnConnected for Installation={InstallationId} Connection={ConnectionId}",
                installationId, Context.ConnectionId);
        }
    ...
    public override async Task OnDisconnectedAsync(Exception? exception)
    {
        ...
        if (installationId.HasValue)
        {
            ...
            {
                logger.LogError(ex, "Error in OnDisconnected for Installation={InstallationId} Connection={ConnectionId}",
                    installationId, Context.ConnectionId);
            }
        }
    ...
    public async Task RegisterInstallation(RegisterInstallationMessage msg)
    {
        ...
        {
            logger.LogError(ex, "Error registering installation Installation={InstallationId} Name={Name}",
                installationId, msg.InstallationName);
            throw;
        }
    }
    ...
    public async Task Heartbeat(HeartbeatMessage msg)
    {
        ...
        {
            logger.LogWarning(ex, "Error processing heartbeat for Installation={InstallationId} Status={Status}",
                installationId, msg.Status);
        }
    }
    ...
    public async Task ReportUpdateProgress(UpdateProgressMessage msg)
    {
        ...
        {
            logger.LogError(ex,
                "Error processing update progress Installation={InstallationId} HistoryId={HistoryId} Phase={Phase} Completed={IsCompleted}",
                installationId, msg.UpdateHistoryId, msg.Phase, msg.IsCompleted);
            throw;
        }
    }
