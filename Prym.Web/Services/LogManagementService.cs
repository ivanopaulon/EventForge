...
public class LogManagementService(
    ...
    ILogger<LogManagementService> logger) : ILogManagementService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var result = await httpClientService.GetAsync<PagedResult<SystemLogDto>>(
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            return await httpClientService.GetAsync<SystemLogDto>($"api/v1/LogManagement/logs/{id}");
        }
        catch (Exception ex)
        {
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var result = await httpClientService.GetAsync<IEnumerable<string>>("api/v1/LogManagement/levels");
            return result ?? ["Debug", "Information", "Warning", "Error", "Critical"];
        }
        catch (Exception ex)
        {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var queryString = BuildQueryString(queryParameters);
            var result = await httpClientService.GetAsync<PagedResult<SanitizedSystemLogDto>>(
