...
# ==============================================================================
#  Prym Client - IIS Full Setup Script
...
$SITE_NAME          = "PrymClient"
$POOL_NAME          = "PrymClient"
$SITE_PORT          = 5240
...
$SITE_CERT_FRIENDLY = "Prym Client IIS"
...
Write-Host "  Prym Client IIS Setup - $(Get-Date -Format 'dd/MM/yyyy HH:mm:ss')" -ForegroundColor Magenta
...
if (!(Test-Path $SITE_PATH)) {
    ...
    Write-FAIL "  dotnet publish Prym.Client -c Release -o $DEPLOY_PATH"
...
$oldHttpRule = Get-NetFirewallRule -DisplayName "Prym Client HTTP $SITE_PORT" -ErrorAction SilentlyContinue
if ($oldHttpRule) {
    Remove-NetFirewallRule -DisplayName "Prym Client HTTP $SITE_PORT" -ErrorAction SilentlyContinue
    Write-OK "Vecchia regola 'HTTP $SITE_PORT' rimossa"
}

$ruleName     = "Prym Client HTTPS $SITE_PORT"
...
if (!$deployOk) {
    ...
    Write-WARN "  dotnet publish Prym.Client -c Release -o $DEPLOY_PATH"
...
Write-Host "  URL server  : https://localhost:$SERVER_PORT  (Prym Server, porta HTTPS)" -ForegroundColor Yellow
...
Write-Host "    dotnet publish Prym.Client -c Release -o $DEPLOY_PATH" -ForegroundColor Cyan
