...
// "Environments:{env}" section — mirrors the same pattern used by Prym.Client.
...
try
{
    ...
    {
        options.ServiceName = "Prym Update Agent";
    });
    ...
    // Lightweight unauthenticated health probe — used by Prym.Server to include
    ...
    // ── Update queue management (called by Prym.Server agent-proxy) ────
    ...
    Log.Information("Prym Update Agent starting. UI at http://localhost:{Port}", earlyAgent.UI.Port);
