            ...
            if (backupPath is not null)
            {
                ...
                catch (Exception rollbackEx)
                {
                    logger.LogError(rollbackEx,
                        "Rollback also failed for {Component} {Version} (PackageId={PackageId})",
        ...
        finally
        {
            ...
            if (zipPath is not null && File.Exists(zipPath))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    catch (Exception deleteEx) { logger.LogWarning(deleteEx, "Last-resort delete also failed for {ZipPath}", zipPath); }
                }
            }

            if (Directory.Exists(tempDir))
                try { Directory.Delete(tempDir, recursive: true); }
                catch (Exception ex) { logger.LogWarning(ex, "Could not delete temp directory {TempDir}", tempDir); }
        }
    }
